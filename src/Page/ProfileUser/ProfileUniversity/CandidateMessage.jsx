...
import {get_all_information_by_id, get_all_messages, get_all_users} from "../../../Redux/actions/MessageThunk";
...
const CandidateMessage = () => {
    ...
    useEffect(() => {
        dispatch(get_all_information_by_id(userPage - 1, pageSize,data.id));
    ...
    useEffect(() => {
        if (!businessId || !businessName) return; // Kiểm tra hợp lệ
        setUsers(prevUsers => {
            let updatedUsers = Array.isArray(prevUsers) ? [...prevUsers] : [...listUsers]; // Đảm bảo dữ liệu từ listUsers

            const exists = updatedUsers.some(user => user.idUser === businessId);
            if (!exists) {
                updatedUsers.push({ idUser: businessId, nameUser: businessName });
            }
            return updatedUsers;
        });

        setActiveUser(businessId);

    }, [businessId, businessName, listUsers]); // Theo dõi thêm listUsers để cập nhật
                                    ...
                                    <Layout style={{flex: 1, display: "flex", flexDirection: "column", height: "100%"}}>
                                        ...
                                        <Content style={{
                                            ...
                                            <div className="chat-intro" style={{
                                                ...
                                                <p> Liên lạc với doanh nghiệp một cách nhanh chóng!</p>
                                            </div>
                                                ...
                                                {messages.map((msg, index) => (
                                                    <div key={index}
                                                        ...
                                                        <strong style={{ color: msg.senderId !== activeUser ? "#111112" : "#111112" }}>
                                                            {msg.senderId !== activeUser ? "Bạn" : "Doanh nghiệp"}:
                                                        </strong> {msg.content}
