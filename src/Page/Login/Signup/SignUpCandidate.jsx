...
import {
    clearResponseBusiness, sign_up_candidate, verify_account_candidate,
...
export function SignUpCandidate() {
    ...
    const response = useSelector((state) => state.UserReducer?.responseCandidate);
    const responseSignUpCandidate = useSelector((state) => state.UserReducer?.responseSignUpCandidate);
    useEffect(() => {
        return () => {
    ...
    useEffect(() => {
        if (responseSignUpCandidate?.code === 200) {
            toast.success('Đăng ký tài khoản thành công' );
            navigate("/");
        } else if (responseSignUpCandidate?.message) {
            toast.error (responseSignUpCandidate.message || 'Mã xác minh không chính xác!' );
        }
    }, [responseSignUpCandidate, navigate]);
    ...
    const handleSendCode = (values) => {
        ...
        const { candidateName, numberPhone, password, confirmPassword,email,province,district,ward } = values;
        ...
        setIsLoading(true);
       dispatch(verify_account_candidate(requestBody));
    };
    ...
    const handleVerifyCodeSubmit = async () => {
        if (!code) {
            ...
            const requestBody = {
                ...
                fullName: formValues.candidateName,
            ...
            dispatch(sign_up_candidate(requestBody));
    };
    const getValidationMessage = (fieldName, value) => {
        switch (fieldName) {
            case 'candidateName':
                if (value.length < 10) return 'Tên ứng viên không được ít hơn 10 ký tự.';
                if (value.length > 256) return 'Tên ứng viên  không được vượt quá 256 ký tự.';
                if (/^\s/.test(value)) return 'Tên ứng viên  không được có dấu cách ở đầu.';
                break;
